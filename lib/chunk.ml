...
| Fun (n, ps, b) ->
  ...
  s.upvalues |> Hashtbl.iter (fun n (i, l) ->
    ...
    | _ -> ());
  add_opcode chunk (GetConstant (Closure {name = n; chunk = c; num_args = List.length ps; num_locals = Hashtbl.length s.locals - List.length ps; upvalues = [||]} |> add_constant chunk)) |> ignore;
...
| Ast.Block [] -> ()
| Block [x] -> tail_compile chunk scope x
...
| If bs ->
  let rec compile_if branches =
    (match branches with
    | [] -> failwith "Empty if"
    | (Ast.Bool true, t) :: _ -> tail_compile chunk scope t
    | (c, t) :: bs' ->
      compile chunk scope c;
      let j = add_opcode chunk (JumpIfFalse 999) in
      tail_compile chunk scope t;
      let j2 = add_opcode chunk (Jump 999) in
      let e = compile_if bs' in
      chunk.code.(j) <- JumpIfFalse (j2 + 1);
      chunk.code.(j2) <- Jump e);
...
let to_closure chunk: Value.closure = {name = ""; chunk; num_args = 0; num_locals = 0; upvalues = [||]}
