...
let rec repl line_num =
  ...
  flush stdout;
  let result = Eval.eval (Lexing.from_channel stdin |> Parser.prog Lexer.read) global_env in
  if result <> VNull then
    Eval.to_string result |> print_endline;
  Env.bind global_env (Printf.sprintf "_%d" line_num) result |> ignore;
  repl (line_num + 1)

let run_file filename = Eval.eval (In_channel.open_text filename |> Lexing.from_channel |> Parser.prog Lexer.read) global_env |> ignore;;
